...
class Monkey():
    ...
    def __str__(self):
        return f'Monkey[{self.id}] items={self.items} op={self.operation} test={self.test} targets={self.true_target},{self.false_target}'
    ...
    def relax(self):    
        self.items = [math.floor(item / 3.0) for item in items]
        return self.items
    ...
    def throw_targets(self):
        return [
            self.get_throw_target(item)
            for item in items
        ]
